...
class OutputAsMutInputsTransformer:
    ...
    def __call__(
        ...
        (
            symbolic_output_shapes,
            input_symbol_map,
        ) = self._infer_output_shapes_with_sources(
        ...
        gm_with_sub.graph.output(
            mut_input_nodes[0] if len(mut_input_nodes) == 1 else tuple(mut_input_nodes)
        )
    ...
    def _infer_output_shapes_with_sources(
        ...
        with FakeTensorMode(shape_env=ShapeEnv()) as mode:
            ...
            symbolic_shapes = [
                list(o.shape)
                for o in (outputs if isinstance(outputs, (tuple, list)) else [outputs])
            ]
        ...
        for item in recipe:
            if isinstance(item, int):
                ...
                if source_node not in cache:
                    cache[source_node] = gm.graph.call_function(
                        getattr, args=(source_node, "shape")
                    )
                shape_attr = cache[source_node]
                dim_val = gm.graph.call_function(
                    operator.getitem, args=(shape_attr, dim_idx)
                )
        ...
        with gm.graph.inserting_before(insert_point):
            for recipe in runnable_shapes:
                ...
                empty_node = gm.graph.call_function(
                    torch.empty, args=(actual_shape,), kwargs={"dtype": dtypes[0]}
                )
